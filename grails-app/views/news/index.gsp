		...
		<div id="list-news" class="content scaffold-list" role="main">
			...
			<thead>
					<tr>
						...
						<th><g:message code="news.author.label" default="Author" /></th>
					
						<g:sortableColumn property="dateCreated" title="${message(code: 'news.dateCreated.label', default: 'Date Created')}" />
					
						<g:sortableColumn property="post" title="${message(code: 'news.post.label', default: 'Post')}" />
				...
				<g:each in="${newsInstanceList}" status="i" var="newsInstance">
					<tr class="${(i % 2) == 0 ? 'even' : 'odd'}">
						...
						<td><g:link action="show" id="${newsInstance.id}">${fieldValue(bean: newsInstance, field: "author")}</g:link></td>
					
						<td><g:formatDate date="${newsInstance.dateCreated}" /></td>
					
						<td>${fieldValue(bean: newsInstance, field: "post")}</td>
